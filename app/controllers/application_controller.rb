...
class ApplicationController < ActionController::Base
  ...
  def initialization
      ...
      #关于博日
      @menu6 = Bioer.find_by_sql("select * from categories where substring(categoryid,1,3) = '006' and length(categoryid) = 6") 
      #交流园地
  ...
  def uploadphotofile(file)
    if !file.original_filename.empty?
    #生成一个随机的文件名
    @filename=getfilename(file.original_filename)
    #向dir目录写入文件
    File.open("#{RAILS_ROOT}/public/productphoto/#{@filename}", "wb") do |f|
    f.write(file.read)
    end
    #返回文件名称，保存到数据库中
    return @filename
